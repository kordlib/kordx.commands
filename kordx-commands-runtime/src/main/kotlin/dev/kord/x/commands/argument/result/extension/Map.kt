...
import dev.kord.x.commands.argument.result.extension.MapResult.Fail
import dev.kord.x.commands.argument.result.extension.MapResult.Pass
...
inline fun <T, R : Any> ArgumentResult<T>.tryMap(mapper: (T) -> MapResult<R>): ArgumentResult<R> =
    when (this) {
        is ArgumentResult.Success -> when (val result = mapper(item)) {
            is Pass -> ArgumentResult.Success(result.item, newIndex)
