package dev.kord.x.commands.argument
...
import dev.kord.x.commands.argument.extension.repeated
 ...
 */
abstract class VariableLengthArgument<T, CONTEXT> :
    Argument<T, CONTEXT> {
    ...
    final override suspend fun parse(
        ...
        val words = text.substring(fromIndex)
            .split(Argument.whitespaceRegex)
     ...
     */
    protected fun <T> success(value: T, wordsTaken: Int): ArgumentResult<T> =
        ArgumentResult.Success(value, wordsTaken)
