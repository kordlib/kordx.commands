package dev.kord.x.commands.argument.result.extension

import dev.kord.x.commands.argument.result.ArgumentResult
import dev.kord.x.commands.argument.result.extension.FilterResult.Fail
import dev.kord.x.commands.argument.result.extension.FilterResult.Pass
 ...
 */
@Suppress("UNCHECKED_CAST")
inline fun <T : Any> ArgumentResult<T?>.filterNotNull(failMessage: (T?) -> String): ArgumentResult<T> =
    when (this) {
        is ArgumentResult.Success -> when (item) {
...
inline fun <T> ArgumentResult<T>.filter(filter: (T) -> FilterResult): ArgumentResult<T> =
    when (this) {
        is ArgumentResult.Success -> when (val result = filter(item)) {
            Pass -> this
            is Fail -> ArgumentResult.Failure(result.reason, 0)
        }
 ...
 */
@Suppress("UNCHECKED_CAST")
inline fun <T> ArgumentResult<T>.filter(
    failIndex: Int,
    filter: (T) -> FilterResult
): ArgumentResult<T> = when (this) {
    is ArgumentResult.Success -> when (val result = filter(item)) {
        Pass -> this
        is Fail -> ArgumentResult.Failure(result.reason, failIndex)
    }
    is ArgumentResult.Failure -> this
}
