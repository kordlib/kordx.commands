package dev.kord.x.commands.argument.state
 ...
 */
fun ParseState.success(): ArgumentResult.Success<String> =
    ArgumentResult.Success(consumed.toString(), cursor)

/**
 * Creates a [ArgumentResult.Success] with the [item] and [ParseState.cursor].
 */
fun <T> ParseState.success(item: T): ArgumentResult.Success<T> =
    ArgumentResult.Success(item, cursor)
 ...
 */
fun <T> ParseState.unexpectedEnd() =
    ArgumentResult.Failure<T>("Expected more input but reached end.", cursor)

/**
 * Creates a [ArgumentResult.Failure] indicating some other [literal] was expected.
 */
fun <T> ParseState.expected(literal: String) =
    ArgumentResult.Failure<T>("Expected '$literal'.", cursor)

/**
 * Creates a [ArgumentResult.Failure] indicating some other [literal] was expected.
 */
fun <T> ParseState.expected(literal: Char) =
    ArgumentResult.Failure<T>("Expected '$literal'.", cursor)
