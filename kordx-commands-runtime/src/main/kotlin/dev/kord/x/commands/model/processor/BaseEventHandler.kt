...
open class BaseEventHandler<S, A, E : CommandEvent>(
    ...
    override suspend fun CommandProcessor.onEvent(event: S) {
        ...
        val command = getCommand(context, commandName) ?: return with(handler) {
            notFound(
                event,
                commandName
            )
        ...
        val arguments = command.arguments as List<dev.kord.x.commands.argument.Argument<*, A>>
        ...
        val eventContext = with(converter) {
            argumentContext.toCommandEvent(
                CommandEventData(
                    command,
                    command.modules,
                    commands,
        ...
        val (items) = when (val result =
            parseArguments(trimmedLength, argString, arguments, argumentContext)) {
    ...
    protected open suspend fun CommandProcessor.parseArguments(
        trimmedLength: Int,
        argumentText: String,
        arguments: List<dev.kord.x.commands.argument.Argument<*, A>>,
        event: A
    ): ArgumentsResult<A> {
        var charIndex = 0
