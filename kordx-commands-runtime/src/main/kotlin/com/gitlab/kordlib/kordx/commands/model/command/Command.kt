 ...
 *
 * @param isCacheEnabled Tells if the command caching should be enabled for this command.
 *
...
data class CommandData<T : CommandEvent>(
        ...
        val execution: suspend (event: T, cache: CommandCache?, arguments: List<*>) -> Unit
)
...
class Command<T : CommandEvent>(val data: CommandData<T>) : KoinComponent {
    ...
    /**
     * Returns the module of this command, found by looking up the [CommandData.moduleName] in [modules].
     *
    ...
    /**
     * Is the cache enabled in the command? If yes, [CommandCache] is provided at invocation of command.
     */
    val isCacheEnabled: Boolean get() = data.isCacheEnabled
     ...
     */
    suspend operator fun invoke(context: T, cache: CommandCache?, arguments: List<*>): Unit =
            data.execution(context, cache, arguments)
}
