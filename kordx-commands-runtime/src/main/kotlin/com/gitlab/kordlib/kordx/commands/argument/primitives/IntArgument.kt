...
open class IntArgument(override val name: String = "Number") : SingleWordArgument<Int>() {
    final override val example: String
        get() = Random.nextInt(-100, 100).toString()

    final override suspend fun parse(word: String, context: ParsingContext): Result<Int> = when (val number = word.toIntOrNull()) {
