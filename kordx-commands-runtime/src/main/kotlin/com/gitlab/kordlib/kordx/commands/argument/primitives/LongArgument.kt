...
open class LongArgument(override val name: String = "Number") : SingleWordArgument<Long>() {
    final override val example: String
        get() = Random.nextDouble(-100.0, 100.0).toString()

    final override suspend fun parse(word: String, context: ParsingContext): Result<Long> = when (val number = word.toLongOrNull()) {
