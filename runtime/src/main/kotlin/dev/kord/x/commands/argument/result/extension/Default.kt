 ...
 */
inline fun <T> ArgumentResult<T>.switchOnFail(generator: () -> ArgumentResult<T>): ArgumentResult<T> =
    when (this) {
...
inline fun <T : Any> ArgumentResult<T?>.orElseSupply(fallback: () -> T): ArgumentResult.Success<T> =
    when (this) {
        is ArgumentResult.Success -> when (item) {
 ...
 */
@Suppress("UNCHECKED_CAST")
fun <T> ArgumentResult<T>.orElse(failIndex: Int, fallBack: T): ArgumentResult.Success<T> =
    when (this) {
        is ArgumentResult.Success -> ArgumentResult.Success(item, newIndex)
        is ArgumentResult.Failure -> ArgumentResult.Success(fallBack, failIndex)
    }
...
inline fun <T> ArgumentResult<T>.orElseSupply(fallBack: () -> T): ArgumentResult.Success<T> =
    when (this) {
        is ArgumentResult.Success -> ArgumentResult.Success(item, newIndex)
        is ArgumentResult.Failure -> ArgumentResult.Success(fallBack(), 0)
    }
